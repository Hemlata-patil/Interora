...
**Project:** Interora — AI-Powered Internship Intelligence & Management Platform
...
## 7.0 UX Navigation & Frontend Information Architecture

### Purpose

The frontend must expose every feature defined in this TRD while keeping the interface simple. **Do not create one sidebar item per database table or feature.** Use grouped navigation and workflow pages.

### Shared shell
- Light theme by default.
- Interora branding and logo used consistently across landing, authentication, dashboards, certificates and public verification.
- White content surfaces on a very light neutral background, with restrained blue/purple brand accents.
- Clear page title + one-line purpose at the top of every screen.
- Primary action is visually obvious; secondary actions are less prominent.
- Tables use filters/search and detail drawers/pages instead of overcrowded dashboards.
- Status is communicated with text + icon + color, never color alone.
- Responsive behavior must preserve the same information hierarchy on desktop, tablet and mobile.

### Sidebar rule
The sidebar contains only these high-level groups. All detailed features remain available inside them:

| Role | Primary navigation groups | Features contained inside |
|---|---|---|
| Student | Home; Internship Journey; My Work; AI & Career; Completion; Profile & Settings | Marketplace, Applications, My Internship, Attendance, Tasks, Milestones, Work Logs, Reports, Feedback, Recommendations, Skill Gap, Learning Hub, Resume Analyzer, Readiness, Leaderboard, Performance, Certificates, Notifications, Profile, Settings |
| Faculty | Home; Approvals; Students & Internships; Monitoring; Insights & Analytics; Completion & Notifications | Applications, Students, Internships, Attendance, Tasks, Milestones, Reports, Cross-Verification, At-Risk, Performance, Placement Analytics, Certificates, Notifications |
| Company | Home; Internships; Applicants & Allocator; My Interns; Internship Operations; Evaluations & Conversion; Company Profile | Listings, Applications, AI Allocator, Interns, Tasks, Milestones, Attendance, Reports, Feedback, Evaluations, Completion, PPO/Conversion, Analytics |
| Admin | Home; People; Internships & Applications; Departments & Companies; Certificates; Analytics & AI; System | Users, Students, Faculty, Companies, Verification, Internships, Applications, Active Internships, Departments, Certificates, Analytics, AI Insights, Settings, Audit Logs |

### Dashboard composition
Every dashboard should answer four questions before showing secondary analytics:
1. **What is my current status?**
2. **What needs my action?**
3. **Is anything at risk?**
4. **Where do I go next?**

Use a small set of KPI cards, one current-state panel, an action queue, alerts/insights, and contextual shortcuts. Detailed modules open from these areas.

### Feature preservation rule
This UX restructuring is **navigation-only**. It must not remove or silently defer any feature already defined in the current technical scope, including AI Intern Allocator, geo/time attendance, proof attachments, AI weekly reports, faculty cross-verification, AI Learning Hub, AI Resume Analyzer, Leaderboard, PPO/conversion tracking, company verification, department management, configurable scoring, or admin feature toggles. Optional/phase-gated items such as the AI chatbot remain optional but must keep their implementation boundary documented.

### Demo-first workflow
For the hackathon, the most visible end-to-end path should be easy to demonstrate:
`Company creates internship → Student applies → Faculty approves → Allocator ranks → Company selects → Internship activates → Student tracks work → Health/Risk updates → Evaluation/cross-verification → Completion → Certificate → QR verification → Readiness/PPO/Leaderboard`.
